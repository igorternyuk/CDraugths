...
BoardDiagonal64::BoardDiagonal64()
{

}

void BoardDiagonal64::SetupInitialPosition()
{
...
bool BoardDiagonal64::IsCoronationTile(int row, int col, Alliance alliance) const
{
    const int HEIGHT = GetBoardHeight();
    const int WIDTH = GetBoardWidth();
    const int N = std::min(HEIGHT, WIDTH);
    bool isInLeftTopCorner = col >= 0 && col < N / 2 && row >= 0 && row < N / 2;
    bool isInRightBottomCorner = col >= N / 2 && col < N  && row >= N / 2 && row < N;
